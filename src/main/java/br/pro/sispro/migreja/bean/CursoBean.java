...
public class CursoBean implements Serializable {
	...
	public void listar() {
		try {
			...
			Messages.addGlobalError("Ocorreu um erro ao tentar listar o curso!");
	...
	public void novo() {
		try {
			...
			Messages.addGlobalError("Ocorreu um erro ao tentar gerar um novo curso!");
			erro.printStackTrace();
			Messages.addGlobalError(erro.getMessage());
		}
	}
	...
	public void salvar() throws Exception {
		...
		try {
			...
			Long codigoRegistro = curso.getCodigo(); //Recupera o ID do Registro do banco de dados
			...
			Messages.addGlobalError("Ocorreu um erro ao tentar salvar um curso!");
			erro.printStackTrace();
			Messages.addGlobalError(erro.getMessage());
		}
	}
	...
	public void excluir(ActionEvent evento) throws Exception {
		...
		try {
			...
			Long codigoRemovido= curso.getCodigo();//Recupera o ID do Registro removido
			...
			if(codigoRemovido != null) {
				...
				Messages.addGlobalInfo("Curso excluído: " + curso.getNomeCurso());			
			}
	...
	public void editar(ActionEvent evento) throws Exception {
		...
		try {
			...
			if(codigoRegistro != null) {
				...
				Messages.addGlobalInfo("O curso foi editado com sucesso!");
			}	
	...
	public void imprimir() {
		...
		try {
			...
			String razaosocialCurso = (String) filtros.get("razaosocial");
			String nomeDepartamentoCurso = (String) filtros.get("nomeDepartamento");
			
			String caminho = Faces.getRealPath("/reports/curso.jasper");
						
			Map<String, Object> parametros = new HashMap<>();
						
			if(razaosocialCurso == null){
				parametros.put("razaosocial", "%%");
			}else{
				parametros.put("razaosocial", "%" + razaosocialCurso + "%");
			}
			if(nomeDepartamentoCurso == null){
				parametros.put("nomeDepartamento", "%%");
			}else{
				parametros.put("nomeDepartamento", "%"+ nomeDepartamentoCurso + "%");
			}
	...
	public void exportarPDF(ActionEvent actionEvent) throws JRException, IOException{
		...
		File jasper = new File(FacesContext.getCurrentInstance().getExternalContext().getRealPath("/reports/curso.jasper"));
		
		Map<String, Object> parametros = new HashMap<>();		
		
		JasperPrint jasperPrint = JasperFillManager.fillReport(jasper.getPath(), parametros, new JRBeanCollectionDataSource(this.cursos));
		
		HttpServletResponse response = (HttpServletResponse) FacesContext.getCurrentInstance().getExternalContext().getResponse();
		response.addHeader("Content-disposition","attachment; filename=Curso.pdf");
	...
	public void verPDF(ActionEvent actionEvent) throws Exception{
		...
		byte[] bytes = JasperRunManager.runReportToPdf(jasper.getPath(), parametros, new JRBeanCollectionDataSource(this.cursos));
		HttpServletResponse response = (HttpServletResponse) FacesContext.getCurrentInstance().getExternalContext().getResponse();
	...
	public void exportarDOC(ActionEvent actionEvent) throws JRException, IOException{		
		...
		HttpServletResponse response = (HttpServletResponse) FacesContext.getCurrentInstance().getExternalContext().getResponse();
		response.addHeader("Content-disposition","attachment; filename=Curso.doc");
		ServletOutputStream outStream = response.getOutputStream();
	...
	public void exportarPPT(ActionEvent actionEvent) throws JRException, IOException{
		...
		HttpServletResponse response = (HttpServletResponse) FacesContext.getCurrentInstance().getExternalContext().getResponse();
		response.addHeader("Content-disposition","attachment; filename=Curso.ppt");
		ServletOutputStream outStream = response.getOutputStream();
